...
// head가 마지막 노드를 가리킨다고 가정한다. 차피 순서가없어서그런가?
...
ListNode* insert_first(ListNode* head, element item)
{
    ...
    ListNode* newNode=(ListNode*)malloc(sizeof(ListNode));
    newNode->data=item;
    // 추가될 새 노드의 다음 노드(본래 첫번째 노드)이고 head가 마지막노드를가리키므로
    if(head==NULL){
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                freedNode=now->next;
                // 만약 마지막 요소를 지우는 거라면 다음 노드 위치가 head가 되야됨
                // 아니면 삭제한 노드의 링크값을 넣어야겟지
                now->next=(pos==get_length(head)-1)?head :freedNode->next;
                // 죽인다
                free(freedNode);
